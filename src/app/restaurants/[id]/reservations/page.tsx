...
export default function ReservationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchReservations = useCallback(async () => {
    ...
    try {
      const res = await fetch(`https://backendpjforfrontend.vercel.app/api/v1/restaurants/${restaurantId}/reservations`, {
        method: 'GET',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleSaveUpdate = async (id: string) => {
    if (!editDate || !editTime) { alert("กรุณาเลือกวันและเวลา"); return; }
    ...
    try {
      const res = await fetch(`https://backendpjforfrontend.vercel.app/api/v1/reservations/${id}`, {
        ...
        body: JSON.stringify({ apptDate: combinedDateTime })
      });
      if (res.ok) {
  ...
  const handleReserve = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!reserveDate || !reserveTime) { alert("กรุณาเลือกวันและเวลา"); return; }
    if (isAdmin && !adminEmail) { alert("กรุณากรอก Email ของลูกค้า"); return; }
    if (!session && !isMockMode) { alert("กรุณา Login"); router.push('/api/auth/signin'); return; }

    const combinedDateTime = `${reserveDate}T${reserveTime}:00`;

    if (isMockMode) { alert("✅ จองสำเร็จ (Mock)"); router.push(`/restaurants?mock=true`); return; }

    const body: any = { apptDate: combinedDateTime, Reservat_at: new Date().toISOString() };
    if (isAdmin) body.userEmail = adminEmail;
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4 flex-col mt-8">
      ...
      {/* Reservation form — shown for both user and admin */}
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md mb-8">
        <form onSubmit={handleReserve} className="space-y-4">

          {/* Admin only — customer email field */}
          {isAdmin && (
            <div>
          ...
          {reserveDate && reserveTime && (
            <p className="text-sm text-gray-500">
              Booking for:{' '}
              <span className="font-semibold text-gray-700">
                {new Date(`${reserveDate}T${reserveTime}`).toLocaleString('th-TH', {
                  dateStyle: 'long',
                  timeStyle: 'short'
                })}
              </span>
              {isAdmin && adminEmail && (
                <> — <span className="text-indigo-600">{adminEmail}</span></>
              )}
            </p>
          )}
          ...
          <button
            type="submit"
            className="w-full bg-gray-900 text-white font-bold py-3 rounded-lg hover:bg-gray-700"
          >
            Confirm Reservation
          </button>
        </form>
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-5xl overflow-x-auto">
        <h3 className="text-xl font-bold mb-4">
          {isAdmin ? "All Reservations" : "Your Reservations Here"}
        </h3>
                  ...
                  <tr key={res._id} className="hover:bg-gray-50 border-b border-gray-200">
                    ...
                    {isAdmin && (
                      <td className="p-3 font-semibold">
                        {res.user?.name || res.user?.email || res.user || 'Unknown'}
                      </td>
                    )}
                    ...
                    <td className="p-3 text-gray-500 text-sm hidden md:table-cell">
                      {bookedAt.toLocaleString('th-TH')}
                    </td>
                    ...
                    <td className="p-3 text-center">
                      {isEditing ? (
                        <div className="flex justify-center gap-2">
                          <button
                            onClick={() => handleSaveUpdate(res._id)}
